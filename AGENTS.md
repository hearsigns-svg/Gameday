...
- **A native Switch ignores an instant synthesized TAP on the iOS 26
  simulator** (2026-09-23/24): the control tool's default tap on a
  UISwitch lands (the system log shows it) and changes nothing — no
  onValueChange. A tap held ≥0.15 s (`duration`), or a short swipe across
  the thumb (left→right on, right→left off), flips it like a finger does.
  Not an app bug; do not "fix" a Switch for it.
