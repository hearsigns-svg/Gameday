...
| Tennis | ATP: Tennis TV ICS (tournament level) + tennisapi1 on RapidAPI (men's draws + order of play → appearances, and the weekly men's ranking; ONE key, `ATP_VENDOR_KEY`) + WTA: api.wtatennis.com (tournaments + draws + order of play) | LIVE in app | Free (tennisapi1 free tier: 50 requests/key/day, measured from its headers 2026-09-02; a slam fortnight budgets at a 15-request peak day) | Current ✓ (78 ATP events; 49 WTA tournaments incl. US Open) | ICS fields-only by ruling; WTA API owner-approved 2026-08-02, no key. Both tours' players get appearances — men's via `pollAtpVendor` (Round 4 item 7, 2026-09-02; replaced the review-sheet chain of Prompt 18), women's via the WTA API. If the men's quota ever binds, the answer is the vendor's paid tier on the same key, never a second free key (owner posture). ITF unavailable (F22). |
